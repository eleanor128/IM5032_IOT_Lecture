...
def set_segments_1(a,b,c,d,e,f,g):
    # 建立一個 dict 將每段對應到狀態
...
def set_segments_2(a,b,c,d,e,f,g):
    # 建立一個 dict 將每段對應到狀態
    states = {'a':a,'b':b,'c':c,'d':d,'e':e,'f':f,'g':g}
    # 對每一段的腳位設定 HIGH（亮）或 LOW（滅）
    for seg, val in states.items():
...
def show_digit_1(n):
    ...
    set_segments_1(*pattern)  # 解包元組並傳給 set_segments_1

# 顯示某個數字 (0~9) 在第二個顯示器
def show_digit_2(n):
    pattern = DIGITS.get(n, (0,0,0,0,0,0,0))  # 取出對應段碼，預設全部熄滅
    set_segments_2(*pattern)  # 解包元組並傳給 set_segments_2
...
try:
    ...
    while True:
        ...
        for n in range(100):
            ...
            show_digit_1(tens)  # 第一個顯示器顯示十位數
            show_digit_2(units) # 第二個顯示器顯示個位數
